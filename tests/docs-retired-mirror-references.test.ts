...
import { extractCitableText, listFiles, REPO_ROOT } from "./helpers/source-scan";
...
//
// The lifted helper (CR-CRU-097 §S6) is excluded for the SAME reason and no
// other: `tests/helpers/source-scan.ts` now carries this guard's own
// derivation comments verbatim — the retired `test_toon.py` /
// `test_probe.py` names quoted as provenance inside listFiles and
// extractCitableText — so scanning it would be this guard reading its own
// commentary back and reporting it as a finding. It is machinery of this
// scan, not a subject of it.
const SELF_SCAN_EXEMPT: Record<string, true> = {
  [join("tests", "docs-retired-mirror-references.test.ts")]: true,
  [join("tests", "helpers", "source-scan.ts")]: true,
};
...
function scanMirrorMentions(): MirrorHit[] {
  ...
  ].filter((abs) => SELF_SCAN_EXEMPT[abs.slice(REPO_ROOT.length + 1)] !== true);
...
function scanDanglingFileCitations(): DanglingCitation[] {
  ...
  const files = listFiles("tests", [".ts", ".py"]).filter(
    (abs) => SELF_SCAN_EXEMPT[abs.slice(REPO_ROOT.length + 1)] !== true,
  );
...
describe("§S2/§S4b guard — no docstring/comment in tests/ cites a test_*.py or *.test.ts filename that does not exist on disk", () => {
  test("zero LIVE dangling test-file citations survive anywhere under tests/ (history-narrated citations are carved out, live ones are not)", () => {
    ...
    // literal — see pythonStatementStrings in tests/helpers/source-scan.ts
    // for why that exclusion is load-bearing rather than an oversight. Zero
    // such docstrings exist under `tests/` today.
    //
