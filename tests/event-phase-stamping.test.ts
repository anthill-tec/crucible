// CR-CRU-057 — §S1: phase must survive the agent. CR-CRU-044 made `phase` a
...
// there is nothing server-side left to classify its work by declared phase.
//
...
// DECLARED PHASE — never a parallel agent lookup, never a name-derived
...
// `phase` column at all, so every assertion on `event.phase` /
// `phase_inferred` / the response's `phase` echo below fails today (the
...
interface RunsPostResponse {
  ...
  // the echo reports "the phase it stamped, alongside CR-056's
  // context.cycleId echo" (a top-level SIBLING to `context`, mirroring how
  // `phase` itself is a top-level sibling of `context` on the Agent type)
...
/**
 * The exact `phase_inferred` field NAME is this CR's to pick. The CR spec
 * names the SQL column `events.phase_inferred`; every prior CR-044/CR-056
 * domain/JSON pair camelCases the equivalent (bound_cycle_id/boundCycleId,
 * first_seen/firstSeen), so `phaseInferred` is the expected JSON key — read
 ...
 */
function phaseInferredOf(e: EventBrief): unknown {
  const rec = e as unknown as Record<string, unknown>;
  return "phaseInferred" in rec ? rec.phaseInferred : rec.phase_inferred;
}
...
const TDD_PHASES = ["RED", "GREEN", "VERIFY"] as const;

describe("CR-CRU-057 §S1 — phase survives the agent (event-level stamping)", () => {
  describe("stamping — a bound TDD-phase agent's run ingest stamps its declared phase onto the stored event", () => {
    for (const phase of TDD_PHASES) {
      test(`registered phase:"${phase}" (bound) -> run ingest's STORED event carries phase:"${phase}" and phase_inferred false/0 (declared, not inferred)`, async () => {
        boot();
        const store = handle!.store;
        const key = seedProject(store);
        const { cycleId } = await fileAndActivate(key, `CR-CRU-057-stamp-${phase}`);
        const agentId = `stamp-${phase}-1`;
        await registerBound(key, agentId, phase, cycleId);
        ...
        // POSITIVE — the exact declared phase, not a guess.
        expect(events[0]!.phase).toBe(phase);
        // POSITIVE (declared, not backfilled) — explicit false/0, not absence.
        expect(isDeclaredNotInferred(phaseInferredOf(events[0]!))).toBe(true);
      });
    }
  });

  describe("the wound this CR closes — a stored event's phase survives the posting agent's unregistration", () => {
    test('RED-phase agent\'s run event still reports phase:"RED" via the events API AFTER the agent unregisters (its row is deleted)', async () => {
      boot();
      const store = handle!.store;
      const key = seedProject(store);
      ...
      const events = await testEventsForProject(key);
      expect(events.length).toBe(1);
      // POSITIVE — phase read back from the STORED EVENT; the agent row is
      // long gone by the time this assertion runs.
      expect(events[0]!.phase).toBe("RED");
    });
  });

  describe("lifecycle events carry the phase too", () => {
    test("the 'registered' lifecycle event stamps the agent's declared phase at registration", async () => {
      boot();
      const store = handle!.store;
      const key = seedProject(store);
      ...
      // POSITIVE — the registration event names the declared phase exactly.
      expect(registered!.phase).toBe("VERIFY");
    });

    test("the 'unregistered' lifecycle event stamps the SAME phase the agent had, even though its row is already deleted by the time the event is queried", async () => {
      boot();
      const store = handle!.store;
      const key = seedProject(store);
      ...
      // POSITIVE — the unregistration event carries the phase captured
      // BEFORE the row was deleted, not a blank/null value.
      expect(unregistered!.phase).toBe("FIX");
    });
  });

  describe("compile and gate ingests stamp phase on the same footing as run ingests", () => {
    test('compile ingest from a registered GREEN-phase agent -> the STORED compile event carries phase:"GREEN"', async () => {
      boot();
      const store = handle!.store;
      const key = seedProject(store);
      ...
      expect(compileEvent!.phase).toBe("GREEN");
    });

    test('gate ingest from a registered VERIFY-phase agent -> the STORED gate event carries phase:"VERIFY"', async () => {
      boot();
      const store = handle!.store;
      const key = seedProject(store);
      ...
      expect(gateEvent!.phase).toBe("VERIFY");
    });
  });

  describe("response echo — the ingest response reports the stamped phase alongside CR-056's context.cycleId echo", () => {
    test("run ingest response carries phase alongside context.cycleId, matching the stored event exactly", async () => {
      boot();
      const store = handle!.store;
      const key = seedProject(store);
      ...
      // POSITIVE — the response NAMES the phase it stamped, alongside the
      // already-established context.cycleId echo.
      expect(body.context?.cycleId).toBe(cycleId);
      expect(body.phase).toBe("RED");

      const events = await testEventsForProject(key);
      expect(events.length).toBe(1);
      // The echo must agree with what actually landed in the feed — never
      // an independent guess.
      expect(body.phase).toBe(events[0]!.phase as string);
    });

    test("gate ingest response ALSO carries phase alongside context.cycleId (the second stamped surface answers identically)", async () => {
      boot();
      const store = handle!.store;
      const key = seedProject(store);
      ...
      expect(body.ok).toBe(true);
      expect(body.context?.cycleId).toBe(cycleId);
      expect(body.phase).toBe("FIX");
    });
  });

  describe("no fabrication — nothing derives phase from the agent id string", () => {
    test("an agent id ENDING '-RED' but registered phase:\"report\" stamps report verbatim, never a name-derived RED", async () => {
      boot();
      const store = handle!.store;
      const key = seedProject(store);
      // Deliberately shaped like a TDD-phase id (the exact CR-CRU-046 stray
      ...
      const events = await testEventsForProject(key);
      expect(events.length).toBe(1);
      // POSITIVE — the declared phase, verbatim.
      expect(events[0]!.phase).toBe("report");
      // NEGATIVE — never the name-shaped guess the id would suggest.
      expect(events[0]!.phase).not.toBe("RED");
    });
  });
});
