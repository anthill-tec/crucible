...
// ── CR-CRU-087 §S1 — the bun toolchain is pinned by ONE declaration ──────
//
// `oven-sh/setup-bun@v2`'s documented resolution order is (1) package.json's
// `packageManager`, (2) package.json's `engines.bun`, (3) `latest`. With no
// `packageManager` field and `engines.bun` set to an open range, every
// setup-bun step in the release workflow resolves whatever bun shipped most
// recently — so CI's toolchain changes with no commit in between, which is
// how a format-parsing suite goes red overnight and skips both publish jobs.
//
// The pin therefore lives in exactly ONE place, and these assertions defend
// that shape structurally (same technique as the bunfig §S1 guard above —
// read the repo's own declarations, never a reviewer eyeballing YAML):
//   - the declaration exists and is EXACT (a range would reintroduce drift);
//   - `engines.bun` stays a RANGE, keeping its separate meaning (the
//     compatibility FLOOR for consumers, vs what we build and test with) so
//     a future "tidy-up" cannot collapse one into the other;
//   - no workflow step carries a second `bun-version:` pin — a pin repeated
//     per job is one place per job to drift;
//   - the declaration equals the bun actually running this suite, so the
//     file and the interpreter can never silently disagree.

function readPackageJson(): Record<string, unknown> {
  return JSON.parse(fs.readFileSync(path.join(REPO_ROOT, "package.json"), "utf8"));
}

function listWorkflowFilesOnDisk(dir: string): string[] {
  const out: string[] = [];
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const full = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      out.push(...listWorkflowFilesOnDisk(full));
...
describe(
  "The bun toolchain is pinned by ONE declaration (CR-CRU-087 §S1) — " +
    "package.json's packageManager, which setup-bun resolves first",
  () => {
    test(
      "AC1 — package.json declares packageManager as an EXACT bun version " +
        "(bun@<major>.<minor>.<patch>): never a range, never `latest`, " +
        "never a caret/tilde",
      () => {
        const declared = readPackageJson().packageManager;

        expect(typeof declared).toBe("string");
        expect(String(declared).split("@")[0]).toBe("bun");
        expect(String(declared)).toMatch(/^bun@\d+\.\d+\.\d+$/);
      },
    );

    test(
      "AC1 — engines.bun is PRESERVED and still a RANGE: the compatibility " +
        "floor for consumers is a different fact from the version we build " +
        "and test with, and neither may be tidied into the other",
      () => {
        const engines = readPackageJson().engines as Record<string, string> | undefined;
        const floor = engines?.bun;

        expect(typeof floor).toBe("string");
        // A floor carries a comparator/wildcard; an exact pin does not. If
        // this ever reads like `1.3.14`, the two fields have been merged and
        // consumers on 1.2 have silently been locked out.
        expect(String(floor)).not.toMatch(/^\d+\.\d+\.\d+$/);
        expect(String(floor)).toMatch(/^(>=?|\^|~|\*)/);
      },
    );

    test(
      "AC1 — the declaration is the SINGLE source: no workflow step sets a " +
        "`bun-version:` input, so a second pin added later breaks this test " +
        "instead of drifting away from package.json in silence",
      () => {
        const workflowFiles = listWorkflowFilesOnDisk(path.join(REPO_ROOT, ".github", "workflows"));

        const withBunVersionInput = workflowFiles
          .filter((file) => /^\s*bun-version\s*:/m.test(fs.readFileSync(file, "utf8")))
          .map((file) => path.relative(REPO_ROOT, file).split(path.sep).join("/"))
          .sort();

        // Named offenders, never a bare count — a substitution must not
        // cancel out. Also proves the assertion is non-vacuous: there IS at
        // least one setup-bun step for the declaration to feed.
        const setupBunSteps = workflowFiles.filter((file) =>
          /uses:\s*oven-sh\/setup-bun@/.test(fs.readFileSync(file, "utf8")),
        );

        expect(setupBunSteps.length).toBeGreaterThan(0);
        expect(withBunVersionInput).toEqual([]);
      },
    );

    test(
      "AC1b — the pin equals the bun actually running this suite: the moment " +
        "the declaration and the interpreter disagree, the pin is a claim " +
        "rather than a fact (this is the local-vs-CI split the CR closes)",
      () => {
        const declared = String(readPackageJson().packageManager ?? "");

        expect(declared.replace(/^bun@/, "")).toBe(Bun.version);
      },
    );
  },
);
