...
`parseJunitPath` call — an abstraction gap that turns into a bug the day a second
...
and v2 `POST /api/v2/runs`) resolve `dataPath` THROUGH the registry entry; a codec
without `parsePath` + a `dataPath` request → 400 `{ok:false, error}` naming the codec.
No direct `parseJunitPath` import remains in `src/server.ts`/`src/v2.ts`.
...
- [ ] grep `parseJunitPath` in `src/server.ts` + `src/v2.ts` returns 0 (registry-only resolution); `src/codecs/index.ts` returns ≥ 1.
