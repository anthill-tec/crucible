...
`CRUCIBLE_WAVE`/`CRUCIBLE_ORCHESTRATOR` when set), read `help[]`/`changed` fields
tolerantly. Behavior flags unchanged — call sites in agent definitions keep working.
