...
| [CR-CRU-041](CR-CRU-041-release-mechanism.md) | Release mechanism (Sandesh-shaped): publishable `@anthill-tec/crucible-server` pkg + `release.yml` trigger repair (push-master → Release → publish) + branch-gated `scripts/release.sh` with pre-tag manifest guard + `RELEASING.md` | feature | IN PROGRESS (0.1.0 blocker) — C1 MERGED a37aaf9 (§S1 publishable `@anthill-tec/crucible-server` + §S6 composite version-pin, early-merge exception); C2 §S2+§S5 trigger topology + `vX.Y.Z`, C3 `release.sh`, C4 `RELEASING.md`, C5 VERIFY still OUTSTANDING on plan 46 | 009 | 4 |
...
| [CR-CRU-045](CR-CRU-045-coverage-shadow-regression.md) | Patch: a `coverage/` dir in the project shadows coverage.py again — CR-040's PYTHONSAFEPATH removal regressed the CR-036 C4 guarantee (bisect-confirmed); restore it without an env var that leaks into grandchild subprocesses | patch | PENDING (0.1.0 blocker) | 040, 036 | 4 |
