...
| [CR-CRU-058](CR-CRU-058-rust-axi-envelope-parity.md) | Verbs that emit no TOON-AXI envelope — confirmed in rust incl. `pre-merge-gate` (the gate an orchestrator reads); fleet-wide extent UNMEASURED, so §S0 builds a detector FIRST and the census scopes the fix; + state-derived `help[]`, stdout purity (absorbs a CR-046 deferral), fleet-wide guard | patch | IN PROGRESS | 030, 054 | 4 |
