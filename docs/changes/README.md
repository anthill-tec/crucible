...
| [CR-CRU-041](CR-CRU-041-release-mechanism.md) | Release mechanism (Sandesh-shaped): publishable `@anthill-tec/crucible-server` pkg + `release.yml` trigger repair (push-master → Release → publish) + branch-gated `scripts/release.sh` with pre-tag manifest guard + `RELEASING.md` | feature | PENDING (0.1.0 blocker) | 009 | 4 |
