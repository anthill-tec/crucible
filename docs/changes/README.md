...
| [CR-CRU-063](CR-CRU-063-ci-provisions-the-toolchain.md) | CI runs the gates but provisions no toolchain — measured on the first real push (run 31677479804): **102 bun + 9 python failures**, 98 of them `Executable not found in $PATH: "uv"`; the Python job is server-free but not dependency-free (`uv`, the declared `dev` extra, `build`). Publishes `needs:` these suites, so a red gate publishes nothing | patch | PENDING | 062 | 4 |
