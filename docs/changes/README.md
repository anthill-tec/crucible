...
| [CR-CRU-065](CR-CRU-065-cause-selection-fits-maven.md) | The no-report cause is picked as the LAST non-empty line — right for python/node (`ModuleNotFoundError` ends those streams), wrong for maven (`cannot find symbol` sits above its `symbol:` lines; the tail is `[ERROR] -> [Help 1]` boilerplate). Additive `cause=` override: selection becomes per-stack, composition/bounding stays shared; head-preserving bound for a supplied cause, tail-preserving for the derived one | patch | PENDING (0.1.0 · release prerequisite) | 064 | 4 |
