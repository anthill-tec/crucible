...
| [CR-CRU-044](CR-CRU-044-phase-as-first-class-data.md) | Patch: agent phase must be declared data, not an agentId naming convention — server REQUIRES the `phase` enum and classifies on it; `--phase` sent by all 5 clients (was discarded); UI reads stored phase, `phaseRole(agentId)` demoted to a history fallback — ⚠ the FIELD was renamed `phase` → `role` by CR-059 (the DN ontology says Role); read this row as history | patch | COMPLETED | 030, 036 | 4 |
...
| [CR-CRU-049](CR-CRU-049-mvn-narration-hardening.md) | Harden `mvn-crucible.py` narration — re-baselined after the audit ran for real: the regex was already correct (verified vs surefire 3.2.5/3.5.4) and the env-quieting half was bun-only, so the live defect was the tautological test fixture. Real surefire format in the fixture + `-B` on all Maven invocations + version pin | patch | COMPLETED | 008, 047 | 4 |
...
| [CR-CRU-052](CR-CRU-052-project-teardown.md) | Projects can be created but never deleted — guarded `DELETE /api/v2/projects/<key>` cascade (403 archived-first + 409 `userApproved`), harness teardown + positive ephemeral guard, residue purged. **Also found and closed a 2nd leak: the e2e suite had been writing to `~/.local/share/crucible/crucible.db` since CR-043 (79 projects/259 events)** | feature | COMPLETED | 012, 032 | 4 |
| [CR-CRU-060](CR-CRU-060-e2e-harness-identity-drift.md) | The e2e harness predated the registered-caller hard stop — `filePlan` sent no `agentId`, other helpers sent unregistered ones. Server was CORRECT (`src/` byte-identical); harness was stale. Idempotent ensure-registered at the helper boundary → **e2e 19 failed/11 passed/10 blocked → 40 passed/0/0**, zero product defects behind the drift | patch | COMPLETED | 056, 052 | 4 |
| [CR-CRU-061](CR-CRU-061-tag-derived-versioning.md) | Bare-SemVer tags end to end (retired the `v` across 6 sites; supersedes CR-041 §S5) + **both artifacts derive their version from the tag** (npm now SETS `package.json`, matching hatch-vcs) + `gate-run --skip` so no-mistakes can run as the release gate + **`finish X.Y.Z` aligns the manifest itself** — one command releases | patch | COMPLETED | 041 | 4 |
| [CR-CRU-062](CR-CRU-062-ci-runs-the-gates.md) | CI published but never tested — added `test-bun`/`test-python`/`test-e2e` + `pack-server`, all with NO event-restricting `if` (⚠ `needs:` does not cross the push/release run boundary, so push-scoped jobs would have made the gate inert), and both publishes now `needs:` all three. **5 facets remain provable only by a real push — see the CR's carry-forward, esp. the deliberately-RED run** | feature | COMPLETED | 041, 052, 060 | 4 |
| [CR-CRU-053](CR-CRU-053-retired-mirror-references.md) | Test headers presented the retired `~/.claude/scripts` mirror as the client source — running it orphans runs. Grew (recorded, incl. one user-decided expansion) to also fix the DN carrying the same false claim, and 9 live citations pointing readers at deleted test files; a whole-tree guard now enforces both, with a per-citing-file history carve-out | patch | COMPLETED | 008, 009, 042 | 4 |
...
| [CR-CRU-014](CR-CRU-014-execution-roadmap.md) | Execution roadmap: queue + Wave/CR table | feature | COMPLETED (0.2.0) | 011, 013 | 5 (0.2.0) |
| [CR-CRU-015](CR-CRU-015-bdd-harness.md) | BDD harness: Playwright runner + codec + tab | feature | PENDING | 004, 007 | 6 (post-0.2.0) |
| [CR-CRU-017](CR-CRU-017-run-lifecycle.md) | Run lifecycle: start/end + Aborted state | feature | COMPLETED (0.2.0) | 008, 011 | 5 (0.2.0) |
| [CR-CRU-018](CR-CRU-018-responsive-mobile.md) | Responsive: mobile + tablet media | feature | PENDING | 016 | 6 (post-0.2.0) |
| [CR-CRU-022](CR-CRU-022-roadmap-analytics.md) | Roadmap analytics: velocity + burndown + forecast — **deferred past 0.2.0**; it owns the confidence-gated P50/P80 forecast, so the roadmap's "possible release date" reading (CR-078 §S2) lands with THIS CR, not in 0.2.0 | feature | PENDING | 011, 014 | 6 (post-0.2.0) |
| [CR-CRU-068](CR-CRU-068-server-discloses-its-store.md) | The server never says which store it opened — `resolveDbPath`'s rule 3 is CWD-relative, so the same binary opens a different DB depending on where it was launched: repo dev server → `data/crucible.db` (9.2 MB, plans=70), installed `crucible-axi serve` from `$HOME` → `~/.local/share/crucible/crucible.db` (60 KB, all zeros). Both bind :3849, so whoever holds the port silently decides where every client writes, and neither startup nor `/api/health` discloses the store. Found by a human noticing absent data — today that is the only way it CAN be found. Disclosure only: resolution order, defaults and rule 3 stay exactly as they are | bugfix | COMPLETED (0.2.0) | 043, 066 | 5 (0.2.0) |
| [CR-CRU-069](CR-CRU-069-uninstall-inverts-install.md) | Install has no inverse — `uninstall` appears zero times in `cli.py`/`install.py`, so `uv tool uninstall crucible-axi` removes only the Python CLI and orphans everything its stages provisioned: the global server pkg (0.1.2), the `crucible-server` symlink on PATH, `~/.crucible` state, and a 9.1 MB store. New `crucible-axi uninstall` verb inverts the stages it owns (state, then server) and `install.sh` gains the inverse path — verb first, `uv tool uninstall` LAST, since a running tool cannot delete itself. `crucible-axi install` stays (install.sh's delegation is the intended path); Bun is never removed. A PLAIN uninstall removes program artifacts ONLY — the store AND the config survive; destroying either needs an explicit `--purge` (interactive runs prompt once, defaulting to retain; non-TTY always retains, so automation cannot silently lose a database) | feature | COMPLETED (0.2.0) | 009, 066 | 5 (0.2.0) |
| [CR-CRU-070](CR-CRU-070-systemd-user-unit.md) | The systemd `--user` unit CR-066 deferred was never filed — yet shipped 0.1.2 forward-references it four times (`cli.py:130`, `install.py:83`, `install.py:397`, CR-066 §deferred), so three shipped comments imply a daemon that does not exist and the only persistent run is a foreground `serve` in a terminal. install.sh provisions the unit (absolute `ExecStart` — a unit inherits no shell PATH, which is why 066 resolves absolutely; `Restart=on-failure`) and reverses it, extending CR-069's inversion to put the unit FIRST (unit → server, with `--purge` appending config → store): tearing the unit down first is what stops systemd restarting a deleted binary. `--user` only, no root; absent systemd skips-with-reason and still exits 0 | feature | COMPLETED (0.2.0) | 066, 069 | 5 (0.2.0) |
| [CR-CRU-071](CR-CRU-071-in-place-upgrade-safe-migration.md) | In-place upgrade has no safe migration — `PRAGMA user_version` is **0** on every store incl. the live 9.1 MB dog-food DB, so `migrate()` infers shape by probing for each column and issuing ad-hoc `ALTER TABLE`s (`store.ts:439-506`). Four gaps: no schema version to ask; **no downgrade guard** (an OLDER server passes every probe on a NEWER store, then reads AND WRITES a schema it does not understand — live corruption path on a version rollback); no recovery point (separate non-atomic `exec`s, nothing copied first); upgrade not gated on migration (a failure surfaces later as a query error). Fix: `user_version` as authority, existing stores baselined without loss, ordered idempotent chain in transactions, WAL-safe `crucible-pre-upgrade-*.db` backup before the first mutating write, newer store REFUSED with both versions + remedy, outcome disclosed at startup and on `/api/health` (CR-068). No down-migrations — reversal is restore-the-backup | feature | COMPLETED (0.2.0 — incl. AC8 upgrade gate + AC9 daemon restart, absorbed from 072) | 001, 043, 068 | 5 (0.2.0) |
| [CR-CRU-072](CR-CRU-072-installer-upgrades-in-place.md) | The installer cannot upgrade — `install.sh:57` runs bare `uv tool install crucible-axi`, which NO-OPS on an existing install (verified in an isolated `UV_TOOL_DIR`: seeded 0.1.1 → `uv tool install` → "Checked 1 package" → still 0.1.1; `uv tool upgrade` → 0.1.2). The README one-liner is also the obvious update command, so every existing 0.1.x user stays stale, and old-code `crucible-axi install` then re-pins the server to the OLD release — both halves frozen while the script prints `bootstrap complete`. Fix: detect an existing install and use uv's own upgrade mechanism, then the custom half (server re-pin per 066, unit refresh per 070, migration gate per 071); report the `0.1.1 → 0.1.2` transition or `already current`; regression test seeds an OLD version in an isolated tool dir and fails a bare `install` | bugfix | COMPLETED (0.2.0 — AC5 → 071 AC8, AC7 → 071 AC9) | 066, 069, 071 | 5 (0.2.0) |
| [CR-CRU-073](CR-CRU-073-gate-events-expire-at-release.md) | Finished releases keep showing their gate — the live store still holds 3 `no-mistakes` events (0.1.0 x2, 0.1.2) for SHIPPED releases, rendering in the Workflow pane as if current. `appendGate` lets them "flow through retention", but retention is a per-project COUNT cap (100, oldest-first), not a lifecycle: a gate leaves only when 100 newer events push it out, and a CURRENT gate can vanish mid-release if the project is busy. Both directions wrong. Fix: a gate names its release and is retired (excluded from the view, kept for audit) in the same transaction that finishes the release; in-flight gates immune to the cap; duplicate runs of one release collapse; the 3 strays retired on migration | bugfix | COMPLETED (0.2.0) | 013, 071, 074 | 5 (0.2.0) |
| [CR-CRU-074](CR-CRU-074-releases-are-first-class.md) | Crucible has never been told a release happened — `MILESTONE_TYPES` (v2.ts:1044) is {gap-analysis, design-review, stage-flip, custom, cr-merged}, every milestone in the live store is `cr-merged`, and the only trace of 0.1.0/0.1.1/0.1.2 is free text inside each gate's `intent`. That absence blocks THREE things: CR-073 has no completion transaction to fire retirement on; CR-014's AC requires wave/**release boundary** divider rows and boundary graph nodes, but waves number continuously across releases and `queue_entries` has no release column; CR-022 specifies `boundaries[]` + a `release:{p50Ts,p80Ts}` forecast band that is underivable without a release record. Adds `release` to the milestone types, reports it from `release.sh finish` AFTER the tag and never blocking on it, a `GET …/releases` read, and backfills the three shipped tags from `git rev-list -1 <tag>` (never the gate text) | feature | COMPLETED (0.2.0) | 013 | 5 (0.2.0) |
| [CR-CRU-076](CR-CRU-076-roadmap-first-tab.md) | **Roadmap is first in the workspace tab band** — the strip is `Workflow · Runs · Coverage · Compile · Roadmap · BDD` with Roadmap FIFTH, but a project begins with roadmap creation and Workflow is only the runtime view of roadmap CRs executing, so the origin document must lead. New order: `Roadmap · Workflow · Runs · Coverage · Compile · BDD`. **Supersedes CR-CRU-021 §S1 AC1** (which deliberately flipped the order TO Workflow-first), so the reversal is explicit, not accidental: `workflow-primary-tab.test.ts` plus `app-logic.test.ts`, `workflow-tab.test.ts`, `pane-scroll-floor.test.ts` and `app-logic.d.mts` pin the old order and are re-targeted with stated reasons. Default LANDING pane is out of scope and unchanged (CR-021 §S1 AC2 stands) | patch | COMPLETED (0.2.0) | 014, 021 | 5 (0.2.0) |
| [CR-CRU-080](CR-CRU-080-release-ceremony-cannot-report.md) | **The release ceremony has never been able to report a release** — `release.sh:346` calls `milestone --type release --label … --commit …` with **no `--agent`**, and the client correctly has no identity fallback (CR-057), so every report dies with `agent-identity-required`. Reporting is non-fatal by design, so the ceremony exits successfully having recorded nothing: proven by running `backfill-releases` for 0.1.0/0.1.1/0.1.2 — three errors, and `GET /releases` still `[]`. That starved everything needing a release record: the roadmap rendered ZERO release boundaries (so nothing distinguished in-release from deferred), CR-073 has no completion event, CR-022 no anchor. Recovered by hand for the three tags; the next release would fail identically. Adds `--agent`/`CRUCIBLE_AGENT`, makes missing identity a PREFLIGHT refusal (before the tag exists), and strengthens the stub test to assert argv actually contains `--agent`. **§S4 (user decision, Option A):** also records `releasedAt` (the tag's own commit date — the stored `timestamp` is ingest time, which is why all three backfills read 2026-08-21 while the tags are 08-19/08-20) and `crs` (the CR ids the tag shipped, from `git log <prev>..<tag>` intersected with the queue). That is the CR→release association CR-077 needs to gate flow; without it nothing says which CRs sit either side of a release diamond | bugfix | COMPLETED (0.2.0) | 074 | 5 (0.2.0) |
| [CR-CRU-082](CR-CRU-082-wave-targets-a-release.md) | **A wave cannot declare the release it targets** — the model is that a release is the milestone ENDING a wave, but Crucible learns of a release only when it **ships** (recorded at tag time), so the wave being executed has no representable terminator: nothing says "wave 5 targets 0.2.0" even though the design board states it. Structural, not cosmetic — CR-077 draws `wave → release → wave`, so the active wave renders as an open end, and CR-078's release reading has nothing to forecast toward. Adds a per-**wave** target release to the queue registration (per-CR would let a wave's CRs disagree about the release they bundle into), kept strictly distinct from shipped releases: a declared target never appears in `GET …/releases`, and when the version ships the real record supersedes the declaration | feature | VOID | 014, 074 | 5 (0.2.0) |
| [CR-CRU-081](CR-CRU-081-release-provenance-uses-ancestry.md) | **Release provenance omits CRs that landed without a naming merge commit** — CR-080 §S4 computes `crs` by matching CR ids in merge-commit **subjects**, so provenance depends on commit-message habits. Measured: `CR-CRU-021` and `CR-CRU-023` are COMPLETED and shipped in 0.1.0, yet `git log --merges --grep` finds nothing for either and they appear in no release's `crs`. CR-080's own F3 already named commit **ancestry** as the exact rule and set it aside on the false belief it needed git in the server — the ceremony has git. Switches to ancestry via the merge sha plans already record, surfaces **unplaceable** CRs instead of dropping them silently, and adds an opt-in repair for the three releases recorded under the old rule (dedup-replay made them immutable, so a re-run cannot fix them) | bugfix | COMPLETED (0.2.0) | 080 | 5 (0.2.0) |
| [CR-CRU-086](CR-CRU-086-repair-must-not-erase-provenance.md) | **The provenance repair ERASED provenance** — dog-fooding CR-081 §S3 on our own releases took `0.1.0` from **58 CRs to 0** (recovered from a pre-run backup; the wiped store kept as `crucible.db.wiped-by-081-repair`). The queue was empty, so the client's queue intersection correctly derived the empty set and the repair **wrote it over 58 good CRs** — the ceremony even printed `crs=(none registered)` first. CR-081 guarded an *absent* field but an **empty list is not absent**: "I have no answer" and "my answer is nothing" were never distinguished on the write path. Invisible to tests because every §S3 fixture seeds a queue, so the destructive path only appears with an unregistered/empty/unreachable queue — exactly a project that just cleared its roadmap. An empty derivation must never overwrite; a repair that cannot compute must refuse loudly; a shrink must be reported | bugfix | COMPLETED | 081 | 5 |
| [CR-CRU-083](CR-CRU-083-derived-status-cannot-say-done.md) | **Derived status cannot say "done"** — `PENDING` means both "never started" and "finished before plan tracking existed", so `CR-CRU-001`–`007` and `010` render PENDING while being long shipped. Self-contradictory, since those CRs are in `0.1.0`'s `crs`: the board claims a CR never started *and* that a release bundled and shipped it. Not fixable by reloading — the queue stores no status by design. Derives from release membership too (per the wave/track/release DN: a release is an activity set that always ships a package), and names the untracked-completion state honestly rather than borrowing the fully-tracked badge | bugfix | COMPLETED | 014, 081 | 5 |
| [CR-CRU-087](CR-CRU-087-ci-bun-is-unpinned.md) | **CI floats to the newest bun, so a format-parsing test flips and blocks every publish** — `test-bun` has been RED in CI since 2026-08-22 while the same suite is green locally (measured: local 1563 passed / 0 failed / 1 skip; CI run 32565982939 `test-bun` failure, every other job green). One assertion is responsible: `tests/clients-bun-crucible.test.ts:700` requires a timed-out leaf to carry NO `failure.message`, and CI returns `"test timed out"`. The marrying parser (`clients/bun-crucible.py:587`) keys on an `error:` block appearing BEFORE the `(fail)` line, which is true on the locally developed bun 1.3.14 and not on the newer bun CI installs — and all five `oven-sh/setup-bun@v2` steps name no `bun-version`, so CI tracks the newest release silently. Since `publish-pypi`/`publish-npm` `needs: test-bun`, **both publish jobs are skipped by construction — 0.2.0 cannot ship**. Pins the toolchain, re-specifies the assertion as leaf ATTRIBUTION rather than absence, and proves both orderings as fixtures | bugfix | COMPLETED (0.2.0) | — | 5 |
| [CR-CRU-088](CR-CRU-088-failure-detail-marries-the-wrong-leaf.md) | **A failure detail printed after its own leaf is attributed to the NEXT test** — `_parse_console_failures` marries an `error:` block to the next `(fail)` line, so a block arriving AFTER its own leaf's fail line and BEFORE the next leaf's is handed to that later leaf. Measured on real bun 1.3.14 output (a leaked async throw): the parser returns `{alpha: expect(...), beta: leaked boom}` — and the wrong message reaches the **ingested tree**, so the board reports one test's failure as another's. The fix needs a discriminator bun's stream does not obviously give (aftermath and prelude sit in the same position), so §S1 settles it on measured evidence — stack-trace correlation, a structured reporter, or stable adjacency. CR-087 pinned the working halves and left this deliberately unguarded; its characterisation test already reproduces it and is replaced here | bugfix | COMPLETED | 087 | 5 |
| [CR-CRU-090](CR-CRU-090-install-lays-the-fleet-down.md) | **`install` never lays the client fleet down, so every manifest path is dead** — `crucible-axi install --help` declares `--target-dir` as "directory the client fleet + manifest are laid down under" and `manifest.build_manifest` publishes `<target-dir>/clients/<stack>-crucible.py` + `<target-dir>/clients/STATUS-CONTRACT.md`, but `STAGE_ORDER = ("server", "manifest")` has no stage that copies anything: the wheel force-includes the fleet as `crucible_axi/clients`, so the manifest is written, converges, and names **six paths that do not exist**. Shipped in 0.1.0 and survived 0.1.1/0.1.2 because CR-CRU-009's AC asked only that the manifest "exists with a stable schema" — satisfiable while every path dangles. An external consumer (Model B) captures this manifest at install pre-flight, so a dead feed is user-visible. Adds a `fleet` stage ordered BEFORE `manifest`, one shared source-dir resolver, and ACs that assert the published paths RESOLVE and a copied client RUNS | hotfix | COMPLETED (0.1.3 · shipped) | — | 5 |
| [CR-CRU-084](CR-CRU-084-release-records-its-packages.md) | **A release does not record what it delivered** — per the wave/track/release DN a release milestone carries a version AND the package(s) of the software developed so far, and always results in a package reaching users. Crucible records `version`/`commit`/`releasedAt`/`crs` but has **no package concept at all** (`grep package src/v2.ts` → 0), so the board can say a release happened but not what a user can install — the one fact a release exists to produce. Every Crucible release ships two version-locked packages (PyPI `crucible-axi`, npm `@anthill-tec/crucible-server`), both known to the ceremony at publish time, neither recorded. Captures what was ACTUALLY published per artifact (0.1.0 shipped to PyPI while npm failed twice — assuming both would have recorded a package that never existed) | feature | COMPLETED | 080, 081 | 5 |
| [CR-CRU-077](CR-CRU-077-roadmap-graph-is-the-execution-dag.md) | The roadmap graph draws a **relationship web, not the plan** — `buildRoadmapGraph` derives edges from `dependsOn` alone, so wave order and release gating are absent, and release diamonds are created as nodes with **zero edges**, leaving them floating beside the graph instead of gating it. Node labels read `title ?? cr`, burying the CR id. Rebuild it as the execution DAG per storyboard F14a: flow composes depends-on + wave order + release boundaries as in-flow gates + parallel fan-out; closed waves collapse to one expandable node; the single active wave gets a cluster box; track lanes are **data-driven** (orchestrator owns the count, never capped in Crucible); active CRs carry live motion; labels lead with the CR id | feature | PENDING (0.2.0) | 014, 076, 080, 083, 084 | 5 (0.2.0) |
...
| [CR-CRU-075](CR-CRU-075-queue-file-fleet-parity.md) | `queue-file` — the Roadmap-registration verb — ships on `python-crucible.py` ONLY, while every other workflow verb (`plan-file`, `milestone`, `cycle-activate`, `cr-close`) is on all five clients. The shared parser already lives in `_crucible_axi.py`, so the gap is purely an unwired subparser; the post-CR-054 fleet census (`THE_42`) predates the verb, so nothing enforced parity. Roadmap registration is a design-phase, any-stack orchestrator action, so python-only is a correctness gap. Wire the thin wrapper into arduino/bun/mvn/rust clients and extend the census to `cmd_queue_file` | patch | PENDING (post-0.2.0) | 014 | 6 (post-0.2.0) |
| [CR-CRU-085](CR-CRU-085-roadmap-multi-track-lanes.md) | **Wave containers + multi-track swimlanes**, split out of CR-077 with approval so its scope is honest. Approved design (roadmap-view DN, decisions 2 and 4) but it delivers nothing to THIS project: a wave is largely an orchestrator synchronization indicator and Crucible is single-track, so data-driven lanes correctly render nothing here — while nested lanes inside a wave container are the layout-fragile part of the graph. Deferred until a genuinely multi-track project needs to read its own board | feature | PENDING (post-0.2.0) | 077 | 6 (post-0.2.0) |
