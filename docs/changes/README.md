...
| [CR-CRU-059](CR-CRU-059-identity-source-validation.md) | Registration identity contract — rename `phase` → **`role`** fleet-wide (the DN's own ontology says Role; `phase` is only the scope word), + validate `identity.source` against its documented enum. Breaking wire change: Model-B intimation BEFORE merge | patch | PENDING | 044, 054, 056, 057 | 4 |
