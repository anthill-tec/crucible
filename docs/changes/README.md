...
| [CR-CRU-061](CR-CRU-061-tag-derived-versioning.md) | **Release blocker.** Bare-SemVer tags (`0.1.0`, no `v` — hardcoded in 6 sites across `release.sh`/`release.yml`) + the npm version DERIVED from the tag instead of hand-bumped (Python already is, via hatch-vcs). Must land before the first tag exists | patch | PENDING | 041 | 4 |
