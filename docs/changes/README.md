...
| [CR-CRU-062](CR-CRU-062-ci-runs-the-gates.md) | **CI publishes but never tests** — `release.yml` is the only workflow and no job runs `bun test`/pytest/e2e. Every gate is local-only, so a tag push publishes whatever is at that commit. Add the three suites + decide what BLOCKS a publish | feature | PENDING | 041, 052, 060 | 4 |
