...
| [CR-CRU-046](CR-CRU-046-toon-conformance.md) | Adopt the official TOON libraries both stacks (`@toon-format/toon` + `toon-format`), delete `clients/toon.py`; resolve deps for path-invoked clients (PEP 723 + `uv run`) | patch | IN PROGRESS (0.1.0 blocker) | 005, 030, 009 | 4 |
...
| [CR-CRU-056](CR-CRU-056-ambiguous-auto-attach-throws.md) | Registered-agents-only server: every mutating verb authenticates a live registration (orchestrators included), registration binds its cycle (validated), ingest never creates agents, auto-attach guessing DELETED | patch | PENDING (P0) | 036, 024, 044 | 4 |
