...
| [CR-CRU-058](CR-CRU-058-rust-axi-envelope-parity.md) | NINE rust verbs emit no TOON-AXI envelope — incl. `pre-merge-gate`, the gate an orchestrator reads; join them to the shared emitters, state-derived `help[]`, stdout purity (absorbs a CR-046 deferral), + a fleet-wide guard | patch | PENDING | 030, 054 | 4 |
