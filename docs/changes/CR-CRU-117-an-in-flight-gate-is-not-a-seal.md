...
- [ ] `workflowLens` does not add a wave to `gatedWaveLabels` for an in-flight gate: asserted with a
      board holding TWO gates on the same `context.wave` — one in-flight, one sealed `passed` — where
      the label is present in the gated set with the seal alone (the anti-vacuity twin) and absent when
      only the in-flight gate is posted. The in-flight fixture MUST carry a `context.wave`: a wave-less
      fixture proves nothing about this exclusion, because the server never stamps one on its own.
- [ ] A wave gated by a real seal is still gated — the narrowing changes no true positive.
- [ ] `boundaryGate` (`public/app.js`) — the SECOND reader that treats the newest scoped gate as a
      verdict, mounting its outcome banner and step ladder as the Workflow tab's primary zone — does
      not select an in-flight gate as the boundary: asserted with an in-flight gate as the newest
      scoped gate and no open plan, the primary zone shows no gate widget for it.
...
- [ ] `map_axi_step_status` never maps an unrecognised or absent step status to `passed`: asserted with
      a status string the mapping table does not name, and with an empty/missing status, neither
      producing a `passed` step. The bias this repairs is the same one `sealed_outcome` was written
      to remove one level up — reintroducing it in the mapper it calls defeats that fix.

**§S3 — fixtures**

- [ ] At least one fixture per driven client is the nine-row shape driven as **non-terminal**:
      `status: running`, no top-level `outcome`, at least one step `pending` — distinct from any
      nine-row fixture already used to drive a SEALING snapshot. The progressive 3/6/8-row fixtures no
      longer stand alone as the only in-flight shape any test sees.
...
Three cycles: the distinction (client + BOTH readers — `workflowLens` and `boundaryGate` — and server
if the mark is stored), the streaming guard with a genuinely non-terminal nine-row fixture per client,
and the step-status mapper's own residual green-biased fallback.
